...
const addressStatus = (status: any, withdrawalAddress) => {
  if (status === undefined || withdrawalAddress?.length === 0) return null;
  if (!status) return <X />;
  return <CheckMark />;
};
...
const columnsData = (props: columnsDataProps) => {
  ...
  return [
    ...
    },
    {
      ...
      title: <HeaderCell>Withdrawal Address</HeaderCell>,
      // compareFunction: (a, b, dir) => compareFunction('status', a, b, dir, 'string'),
      valueRender: (_a, _b, publicKey) => (
        <WithdrawalAddressWrapper>
          <WithdrawalAddressInput
            type={'text'}
            value={validators[publicKey]?.withdrawalAddress ?? ''}
            onChange={(e: any) => onChangeAddress(e.target.value.trim(), publicKey)}
          />
          {addressStatus(validators[publicKey]?.addressStatus, validators[publicKey]?.withdrawalKey)}
          {validators[publicKey]?.withdrawalKey && (
            <ApplyToAll onClick={() => applyToAll(validators[publicKey]?.withdrawalKey)}>
              Apply to all
            </ApplyToAll>
          )}
        </WithdrawalAddressWrapper>
